...
var remoteBranch = process.env.PUPPETFILE_GIT_DEFAULT_BRANCH || 'production';
...
// Clone function
var clone = function *(gitdir, pfileRepo) {
...
var checkoutExisting = function *(branchName, gitopts) {
  ...
  // We merge potential changes from $remoteBranch branch
  logger.debug('Merging potential changes from '+ remoteBranch +' branch, prioritizing our version in case of conflicts');
  var merge_out = yield git('merge '+remoteName+'/'+ remoteBranch +' -X ours --no-edit', gitopts);
