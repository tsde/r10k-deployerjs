...
var queue = kue.createQueue({
  prefix: process.env.REDIS_KEY_PREFIX || 'q',
  redis: {
...
var processJob = function() {
  queue
    .on('job complete', function(id) {
      logger.info('Redis task with ID '+id+' successfully completed');
    })
    .on('job failed attempt', function(id, errmess) {
      logger.warn('Redis task with ID '+id+' failed at first attempt. Reason: '+errmess);
    })
